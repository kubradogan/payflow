...
class EnhancedRouter(
    ...
    fun chooseProvider(): Decision {
        val candidates = listOf(stripe.name to stripe, mock.name to mock).filter { (n, _) -> health.isUp(n) }
        if (candidates.isEmpty()) return Decision("stripe", stripe, "all-down-degrade")
        ...
        for ((name, prov) in candidates) {
            ...
            val costWeight = if (name == "mockpsp") 0.5 else 0.8

            val score = 0.4 * successRate + 0.4 * (1 - latencyNorm) + 0.2 * costWeight
            if (score > bestScore) {
                bestScore = score; best = name to prov
            }
        }
        val (pname, p) = best!!
    ...
    fun report(name: String, success: Boolean, latencyMs: Long) = stats.report(name, success, latencyMs)
}